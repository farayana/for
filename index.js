...
function postDate(sa, countryName, isAbbreviated) {
    ...
    var sa = sa || 'dd.mm.yyyy',
        ...
        months, 
        countryName = countryName || 'hu',  // Мова для місяців. 
        isAbbreviated = isAbbreviated || false, // Скорочений варіант місяців до трьох букв
        localDate = new Date();
    ...
    function format(date, formatstring, separator) {
        var innerDate = "",
            day = date.getDate(),
    ...
    $(document).ready(function () {
      ...
      $('.reviews-form').submit(function () {
        if (Number(inputAge.val()) > 17 && Number(inputAge.val()) < 91 && reviewsInputTextarea.val().length !== 0 && reviewsInputName.val().length !== 0) {
          ...
          fileFlag = true;
          reviewsInput.val('');
          fileText.html('Fényképe feltöltése');
          fileImg.show();
          fileIcon.hide();
  ...
  $(document).ready(function () {
    ...
    setInterval(function () {
      ...
      if (counter == 1) {
        $(".form__visitors p").fadeOut(1100, function () {
          $(this).text('116 eladás az utóbbi órában');
        }).fadeIn();
        counter = 0;
      } else {
        $(".form__visitors p").fadeOut(1100, function () {
          $(this).text('Most a weboldalon ' + randNum + ' látogató');
        }).fadeIn();
        counter = 1;
      }
    }, 6000);
  ...
  $(window).on("scroll resize", function () {
    if (!isNavLink) {
      hideBannerScroll(".x_order_form, .action, .action_bg, .action-2"); // сюда через кому дописываем классы блоков, которые должны скрыть баннер
    }
  });
